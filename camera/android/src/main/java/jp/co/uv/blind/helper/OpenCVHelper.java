...
public class OpenCVHelper extends Imgproc {
	...
	public void findLargestRectangle(Context context, Mat original_image, int videoWidth, int videoHeight, final EventChannel.EventSink decode) {
...
//		Log.d("@@@", String.valueOf(tl.x) + " + " + String.valueOf(tl.y));
...
//		Imgcodecs.imwrite(filename, undistorted);
		...
		byte[] output = new byte[(int) (undistorted.total() * undistorted.channels())];
		undistorted.get(0, 0, output);
		...
		Decode a = new Decode(context,output, undistorted.width(), undistorted.height());
		Log.d("@@@", tl.x + "," + tl.y + "+" + tr.x + "," + tr.y + "+" + bl.x + "," + bl.y + "+" + br.x + "," + br.y);
		String temp =  a.decodeImage();
