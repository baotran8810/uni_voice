...
//        ImageReader.newInstance(previewSize.getWidth(), previewSize.getHeight(), imageFormat, 1);
    imageStreamReader =
            ImageReader.newInstance(480, 720, imageFormat, 1);
  ...
  private void setImageStreamImageAvailableListener(final EventChannel.EventSink imageStreamSink) {
    imageStreamReader.setOnImageAvailableListener(
        reader -> {
          ...
          Log.d("@@@", String.valueOf(img.getHeight()) + "x" + String.valueOf(img.getWidth()));
          ByteBuffer bufferImage = img.getPlanes()[0].getBuffer();
          byte[] data = new byte[bufferImage.remaining()];
          bufferImage.get(data, 0, data.length);

          Mat bufferFrame = new Mat(img.getHeight(), img.getWidth(), CvType.CV_8UC1);
          bufferFrame.put(0, 0, data);
          ...
          mBackgroundHandler.postAtFrontOfQueue(new Runnable() {
            ...
            public void run() {
              if(bufferFrame != null){
                mOpenCVHelper.output = mOpenCVHelper.detectEdges(bufferFrame);
              }
            }
          });


          if(mOpenCVHelper != null && mOpenCVHelper.mPointsOfRectangle != null && mOpenCVHelper.mPointsOfRectangle.length == 4  && mOpenCVHelper.mPointsOfRectangle[0].x > 0 ) {
            //Log.d("Point", String.valueOf(mOpenCVHelper.mPointsOfRectangle[0].x) + "x" + String.valueOf(mOpenCVHelper.mPointsOfRectangle[0].y));
            ...
            if (mOpenCVHelper.output != null) {
              Mat outputFrame = mOpenCVHelper.output;
